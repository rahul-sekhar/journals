namespace :backups do
  namespace :create do
    ...
    def create_local_backup
      ...
      sh "pg_dump -h localhost -U #{sensitive_settings['db_username']} #{sensitive_settings['db_name']}_#{Rails.env} > db-bak.sql"
      print_msg "Storing the database and shared assets in the backups folder"
      filename = "bak.#{Time.now.to_i}.tar.gz"
      sh "tar czhf backups/#{filename} db-bak.sql #{app_settings['shared_assets'].join(" ")}"
      return filename
    end
  end
...
def sensitive_settings
  @sensitive_settings ||= YAML::load_file("config/sensitive.yml")[app_settings['app_name']]
end
